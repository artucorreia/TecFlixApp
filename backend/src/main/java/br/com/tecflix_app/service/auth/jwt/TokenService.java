...
public class TokenService {
  ...
  public TokenDTO generateToken(UUID userId) {
    ...
    try {
      ...
      return new TokenDTO(
          userId, token, refreshTokenService.create(userId).getToken(), createdAt, expiresAt);
  ...
  public UUID validateToken(String token) {
    ...
    try {
      Algorithm algorithm = Algorithm.HMAC256(secretKey);
      subject = JWT.require(algorithm).withIssuer("TecFlixApp").build().verify(token).getSubject();
