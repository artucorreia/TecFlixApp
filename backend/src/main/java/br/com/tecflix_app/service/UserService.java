...
public class UserService {
  ...
  // NOTE: This function is used when user is logged and wants to change their password
  @Transactional(rollbackFor = Exception.class)
  ...
  // NOTE: This function is used when the user has forgotten his password
  @Transactional(rollbackFor = Exception.class)
  ...
  public GenericResponseDTO<UUID> createProfessor(RegisterProfessorDTO data) {
    LOGGER.info("Changing user type to 'professor'");

    UserDTO user = new UserDTO(tokenService.getUserId());
    validatorService.checkIfUserAlreadyHasProfessorRegistration(user.getId());

    // NOTE: Save professor data and socials
