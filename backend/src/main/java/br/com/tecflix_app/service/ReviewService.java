...
public class ReviewService {
  ...
  public List<ReviewDTO> findByCourseId(UUID courseId) {
    ...
    // NOTE: Check if courseId is valid
    courseService.findById(courseId);

    return mapper.map(repository.findByCourseId(courseId), ReviewDTO.class);
  }
  ...
  public GenericResponseDTO<Long> create(UUID courseId, CreateReviewDTO data) {
    ...
    data.setUser(new UserDTO(userId));

    data.setCourse(new CourseDTO(courseId));
